...
from src.orchestrator import task_board, concerns_board, knowledge_board, repo_search
from src.orchestrator.delegator import decide, build_contract, build_compact_contract, build_compact_contract_json
...
def run_orchestrate(argv: list[str]) -> None:
    ...
    from src.orchestrator.knowledge_parser import VALID_CATEGORIES as _KB_VALID_CATEGORIES
    ...
    if args.action == "list":
        ...
        from src.orchestrator.status_report import render_markdown
        tasks = task_board.list_tasks()
        print(render_markdown(tasks))

    elif args.action == "briefing":
        from src.orchestrator.briefing import build_briefing, build_briefing_json
        tasks = task_board.list_tasks()
        ...
        try:
            from src.orchestrator.delegator import _get_relevant_knowledge_for_task
        ...
        from src.orchestrator.manager_loop import ManagerLoop
...
def concerns_report_icon(severity: str) -> str:
    from src.orchestrator.concerns_report import SEVERITY_ICON
...
def _run_concern(args) -> None:
    ...
    if action == "file":
        ...
        from src.orchestrator.concern_parser import get_concern_section_text
        try:
        ...
        from src.orchestrator.concerns_report import BOARD_MD_PATH
...
def _run_knowledge(args) -> None:
    ...
    if action == "file":
        ...
        from src.orchestrator.knowledge_parser import get_knowledge_section_text
        try:
