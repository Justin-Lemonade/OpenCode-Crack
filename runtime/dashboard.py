"""Modern read-only local dashboard for AI-Brain + OpenCode Swarm.

Run with::
    python -m src.runtime.dashboard
...
from src.config import CONTROL_DB_PATH, SWARM_DB_PATH
...
<title>AI-Brain Swarm Control</title>
<style>
...
<header><div><h1>AI-Brain Swarm Control</h1><div class="sub">Agent roles, exact task ownership, live sessions, communication and runtime state</div></div><div class="refresh">Auto-refresh: 5s · <span id="updated">loading</span></div></header>
...
def serve(host: str = HOST, port: int = PORT) -> None:
    ...
    print(f"AI-Brain Swarm dashboard: http://{host}:{port}")
    try:
